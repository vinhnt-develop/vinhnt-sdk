  ...
  async executeToolCalls(
    ...
    for (let batchStart = 0; batchStart < toolCalls.length && !runAbort.signal.aborted && toolCallCount < limit; batchStart += limit) {
      ...
      const tasks = execBatch.map((tc) => async () => {
        ...
        // its side effects, while the promise still hard-caps at the deadline.
        const execPromise = toolTimeout
          ? withToolTimeout(
              (signal) => tool.execute(execInput, { ...toolCtx, signal }),
              toolTimeout,
              runAbort.signal,
              `Tool "${tc.toolName}" timed out after ${toolTimeout}ms`,
            )
