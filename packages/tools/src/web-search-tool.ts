...
export function createWebSearchTool(config: WebSearchToolConfig) {
  return defineTool<{ query: string; numResults?: number | undefined; searchDepth?: "basic" | "advanced" | undefined }, {
    results: SearchResult[];
    ...
    async execute(v, _ctx) {
      ...
      try {
        const data = await config.provider.search(v.query, {
          numResults: v.numResults ?? config.defaultNumResults ?? 5,
          ...(v.searchDepth !== undefined ? { searchDepth: v.searchDepth } : {}),
        });
