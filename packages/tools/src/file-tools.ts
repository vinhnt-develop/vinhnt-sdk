 ...
 * truncated/partial destination file.
 */
async function atomicWriteFile(filePath: string, content: string): Promise<void> {
  const tmp = join(dirname(filePath), `.${basename(filePath)}.tmp-${process.pid}-${Math.random().toString(36).slice(2)}`);
...
/**
 * Resolve `target` to its real path using the deepest existing ancestor.
 * If the file does not exist yet (e.g. a write to a new file), the deepest
 * existing ancestor is realpath'd and the remaining segments are re-appended
 * lexically. This prevents symlink/junction escapes: a symlink inside the
 * workspace pointing outside resolves to a path that is no longer contained.
 */
  ...
  for (;;) {
    try {
      ...
      if (err && (err as NodeJS.ErrnoException).code === "ENOENT") {
        ...
        if (parent === current) {
          throw new ToolPermissionDenied(current, "Path has no existing ancestor within the workspace");
        }
...
/**
 * Verify `target` stays inside the (realpath'd) workspace and return the real
 ...
 */
async function ensurePathAccess(target: string, root: string, pathLabel: string, ctx: ToolContext, externalDirAccess?: boolean): Promise<string> {
  const realRoot = await resolveRealPath(root);
  const realTarget = await resolveRealPath(target);
  if (isWithinWorkspace(realTarget, realRoot)) return realTarget;
  if (isWithinWorkspace(target, root)) {
    throw new ToolPermissionDenied(pathLabel, "Path resolves outside workspace via symlink — access rejected");
  }
  if (externalDirAccess) return realTarget;
...
export function createReadFileTool(workspaceRoot: RootGetter, tracker?: FileReadTracker, externalDirAccess?: boolean, maxFileSize?: number) {
  return defineTool<{ filePath: string; stripTrailingNewline?: boolean }, string>({
    ...
    async execute(v, ctx) {
      ...
      await tracker?.trackRead(target, st);
  ...
  return defineTool<{ filePath: string; content: string }, {
    ...
    async execute(v, ctx) {
      ...
      ctx.setCompensation(async () => {
        if (oldContent) {
          await atomicWriteFile(target, oldContent);
        } else {
...
export function createEditFileTool(workspaceRoot: RootGetter, tracker?: FileReadTracker, externalDirAccess?: boolean) {
  return defineTool<{ filePath: string; oldString?: string | undefined; newString?: string | undefined; edits?: readonly { oldString: string; newString: string }[] | undefined }, {
    edited: string; diff: string;
    hunkCount?: number | undefined; additions: number; removals: number;
  }>({
    name: "edit_file",
    ...
    async execute(v, ctx) {
      ...
      if (v.edits && v.edits.length > 0) {
        ...
        await atomicWriteFile(target, result.newContent);
...
export function createListDirectoryTool(workspaceRoot: RootGetter, externalDirAccess?: boolean, excludedDirs?: string[]) {
  const dirsToExclude = excludedDirs ?? DEFAULT_EXCLUDED_DIRS;

  return defineTool<{ dirPath: string }, { name: string; type: string; path: string }[]>({
    name: "list_directory",
    description: `List files and directories in a path (non-recursive). Skips ${dirsToExclude.join(", ")}.`,
    risk: "read",
    ...
    async execute(v, ctx) {
      ...
      return entries
        .filter((e) => !dirsToExclude.includes(e.name))
        .map((e) => ({
          name: e.name,
