    ...
    return {
      ...
      settle: async ({ name, args, ctx }) => {
        ...
        const defaultCtx: ToolContext = {
          ...
          env: {},
          // RV-41: standalone settle is fail-closed — without a wired permission
          // gate there is no human in the loop, so a tool's `ask` must reject
          // rather than auto-approve itself.
          ask: async () => "reject",
          metadata: () => {},
          setCompensation: () => {},
        };
