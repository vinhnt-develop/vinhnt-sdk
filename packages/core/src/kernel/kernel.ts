...
import {
  ...
  emitCompletedWithSession as sessionEmitCompleted,
...
/** Mirrored pending-input entry (RV-21). `persisted` resolves once the row exists. */
interface PendingInputEntry {
  text: string;
  persisted?: Promise<{ sessionId: string; messageId: string; admittedSeq: number }>;
}
...
export class AgentKernel {
  ...
  private readonly runSessionIds = new Map<RunId, string>();
  /** FIFO mirror of the state-machine input queue, used to mark promotion (RV-21). */
  private readonly runPendingEntries = new Map<RunId, PendingInputEntry[]>();
  /** Per-session monotonic admitted-seq allocator, seeded from the store lazily (RV-21). */
  private readonly nextAdmittedSeq = new Map<string, number>();
  /** Per-session serialized write chain so admitted seqs stay FIFO-consistent (RV-21). */
  private readonly inputWriteChains = new Map<string, Promise<unknown>>();
...
this.stepExecutor = new StepExecutor({
  ...
  async runAgent(agentId: AgentId, prompt: string, ctx: RequestContext, sessionId?: string, parentRunId?: RunId, signal?: AbortSignal): Promise<string> {
    if (!this.agentRegistry) throw new KernelError("internal_error", "No agent registry configured");
  ...
  private buildSystemPrompt(agent: AgentConfig | undefined): string | undefined {
  ...
  run(prompt: string, ctx: RequestContext, sessionId?: string, userContentParts?: readonly MessageContentPart[], agentOverride?: AgentConfig): RunHandle {
    ...
    try {
      ...
      this.runSessionStates.set(runId, this.sessionState);
        ...
        if (sessionId) this.runSessionIds.set(runId, sessionId);
  ...
  async resumeRun(
    ...
    try {
      ...
      if (resolvedSessionId) this.runSessionIds.set(runId, resolvedSessionId);
  ...
  createRunHandle(
    ...
    const runPromise = (async () => {
      try {
        ...
        if (abortController.signal.aborted) abort.abort();
  ...
  private queuePendingEntry(runId: RunId, text: string): void {
    const sessionId = this.runSessionIds.get(runId);
    const list = this.runPendingEntries.get(runId) ?? [];
    let persisted: PendingInputEntry["persisted"];
  ...
  private persistPendingInput(sessionId: string, text: string): Promise<{ sessionId: string; messageId: string; admittedSeq: number }> {
  ...
  private async maxAdmittedSeq(sessionId: string): Promise<number> {
    if (typeof this.sessionStore?.listMessages !== "function") return 0;
    const messages = await this.sessionStore.listMessages(sessionId);
    let max = 0;
    for (const m of messages) {
      if (typeof m.admittedSeq === "number" && m.admittedSeq > max) max = m.admittedSeq;
    }
    return max;
  }
   ...
   */
  private async hydratePendingInputs(runId: RunId, sessionId: string | undefined): Promise<void> {
    if (!this.sessionStore || !sessionId) return;
    if (typeof this.sessionStore.listMessages !== "function") return;
    const messages = await this.sessionStore.listMessages(sessionId);
    let max = 0;
    ...
    const pending = messages
      .filter((m) => typeof m.admittedSeq === "number" && (m.promotedSeq === undefined || (m.promotedSeq as number) < (m.admittedSeq as number)))
      .sort((a, b) => (a.admittedSeq ?? 0) - (b.admittedSeq ?? 0));
    const list = this.runPendingEntries.get(runId) ?? [];
    for (const m of pending) {
      this.stateMachine.sendInput(runId, m.content);
      list.push({ text: m.content, persisted: Promise.resolve({ sessionId, messageId: m.id, admittedSeq: m.admittedSeq as number }) });
    }
  ...
  private async markInputsPromoted(runId: RunId, texts: string[]): Promise<void> {
    ...
    try {
      const result = await executeRunLoop(orchestratorDeps, {
        ...
        emitCompleted: (event, sid, rid, totalIn, totalOut, status) =>
          this.emitCompleted(event, sid, rid, totalIn, totalOut, status),
  ...
  private emitCompleted(event: { id: string; runId: RunId; type: string; occurredAt: string; traceId: string; data: Record<string, unknown> }, sid: string | undefined, rid: RunId, totalIn: number, totalOut: number, status: string): Promise<void> {
    return sessionEmitCompleted(this.sessionDeps, event as unknown as Omit<KnownRunEvent, "sequence">, sid, rid, totalIn, totalOut, status);
  }

  private emitFail(rid: RunId, c: RequestContext, reason: string, steps: number, sid?: string, totalIn = 0, totalOut = 0, dur?: number, cancelled = false): Promise<void> {
