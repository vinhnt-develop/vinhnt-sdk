...
import { ensurePathAccess, resolveRoot, type RootGetter } from "@vinhnt-sdk/tools";
...
export function createCreateSkillTool(workspaceRoot: RootGetter = () => process.cwd(), externalDirAccess?: boolean) {
  return defineTool<CreateSkillInput, string>({
    ...
    async execute(v, ctx): Promise<string> {
      const root = resolveRoot(workspaceRoot);
      const dir = v.directory ?? join(".vnt", "skills", v.name);
      const targetDir = await ensurePathAccess(resolve(root, dir), root, dir, ctx, externalDirAccess);
