...
export interface RunEventStore {
  ...
  /**
   * Append an event and apply session updates atomically (in a DB transaction when available).
   * Falls back to sequential execution if the store doesn't support transactions.
   */
  appendTransactional?(event: RunEvent, sessionUpdate?: { sessionId: string; updates: SessionUpdates }): Promise<void>;
  /** Check if an event with the given ID already exists (idempotency guard). */
