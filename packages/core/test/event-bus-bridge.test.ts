...
import type { EventDefinition } from "@vinhnt-sdk/event";
import type { RunId } from "@vinhnt-sdk/schema";

type DurableDef = EventDefinition<unknown> & { durable: NonNullable<EventDefinition["durable"]> };
...
describe("EventBusBridge", () => {
  ...
  it("persists durable events to store with correct sequence", async () => {
    ...
    bridge.publish(StepStarted, { step: 1 }, { aggregateId: "run-1" });
  ...
  it("assigns monotonically increasing sequences per aggregate", async () => {
    ...
    for (let i = 0; i < 5; i++) {
      bridge.publish(StepStarted, { step: i }, { aggregateId: "run-1" });
  ...
  it("sequences are independent per aggregate", async () => {
    ...
    await new Promise((r) => setTimeout(r, 20));
    bridge.publish(StepStarted, { step: 2 }, { aggregateId: "run-1" });
    await new Promise((r) => setTimeout(r, 50));
...
describe("EventBusBridge.durable()", () => {
  it("replays stored events then live events", async () => {
    ...
    await store.append({
      ...
      traceId: "t-1" as never,
      data: { prompt: "stored" },
    });

    const gen = bridge.durable(RunStarted as DurableDef, "run-1");
    ...
    expect((results[0] as { data: { prompt: string } }).data).toEqual({ prompt: "stored" });
  });
  ...
  it("replays events after a given sequence", async () => {
    ...
    await store.append({
      ...
      type: "run.started",
      occurredAt: new Date().toISOString(),
      traceId: "t-2" as never,
      data: { prompt: "new" },
    });

    const gen = bridge.durable(RunStarted as DurableDef, "run-1", 1);
    const results: unknown[] = [];
    for await (const ev of gen) {
      results.push(ev);
    ...
    expect(results).toHaveLength(1);
    expect((results[0] as { data: { prompt: string } }).data).toEqual({ prompt: "new" });
  });
});