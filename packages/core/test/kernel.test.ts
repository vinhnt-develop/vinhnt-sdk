...
import type { AgentId, AgentConfig, ModelProvider, ModelResponse, ModelStreamEvent } from "@vinhnt-sdk/core";
import type { RunEvent, KnownRunEvent, TokenStreamedData, ThinkingCompletedData, ToolSelfCorrectingData, RequestId, RunId, TraceId } from "@vinhnt-sdk/schema";
import type { CircuitBreakerOptions } from "@vinhnt-sdk/step-executor";

const testCtx = {
  requestId: "test-req-1" as RequestId,
  traceId: "test-trace-1" as TraceId,
...
describe("AgentKernel", () => {
  it("TC00: run-loop budgets from provider usage, not local countTokens (RV-42)", async () => {
    class UsageModel extends FakeModelProvider {
      override async *stream(request: ModelRequest, signal?: AbortSignal): AsyncIterable<ModelStreamEvent> {
        for await (const e of super.stream(request, signal)) yield e;
        yield { type: "usage", inputTokens: 7, outputTokens: 3 };
      }
    }
    const model = new UsageModel([{ content: "Hello!" }]);
    ...
    const data = completed?.data as { inputTokens?: number; outputTokens?: number };
    expect(data.inputTokens).toBe(7);
    expect(data.outputTokens).toBe(3);
    const counted = events.filter((e) => e.type === "token.counted");
    expect(counted).toHaveLength(1);
  });

  it("TC01: complete run with text response â€” no tool call", async () => {
  ...
  it("TC02: tool call success â€” LLM calls 1 tool, gets result, responds", async () => {
  ...
  it("TC04: maxSteps exceeded â€” agent loop stops with status failed", async () => {
  ...
  it("TC04c: per-run saga â€” compensation registered on a tool fires when the run fails (rollbackAll)", async () => {
  ...
  it("TC04d2: saves a final snapshot with status complete before completing (RV-33)", async () => {
    const model = new FakeModelProvider([{ content: "Finished" }]);
    const store = new FakeRunEventStore();
    const kernel = new AgentKernel({ model, store, tools: [], maxSteps: 10 });
    ...
    const snapshot = await store.getSnapshot(handle.runId);
    expect(snapshot).not.toBeNull();
    expect(snapshot?.state.step).toBeGreaterThanOrEqual(1);
    expect(snapshot?.state.status).toBe("succeeded");
    expect(snapshot?.state.finalOutput).toBe("Finished");
  });

  it("TC04d3: resumeRun rebuilds a run from durable storage and continues (RV-34)", async () => {
    const model = new FakeModelProvider([{ content: "First answer" }]);
    const store = new FakeRunEventStore();
    const kernel = new AgentKernel({ model, store, tools: [], maxSteps: 10 });

    const handle = kernel.run("Initial prompt", testCtx, "resume-sess-1");
    await handle.completed;

    // Original run emitted exactly one run.started and reached terminal state.
    const eventsBefore = await store.list(handle.runId);
    expect(eventsBefore.filter((e) => e.type === "run.started").length).toBe(1);
    expect(eventsBefore.some((e) => e.type === "run.completed")).toBe(true);

    const snapshotBefore = await store.getSnapshot(handle.runId);
    expect(snapshotBefore?.state.status).toBe("succeeded");

    // Simulate a process restart: a brand-new kernel over the same store.
    const resumedModel = new FakeModelProvider([{ content: "Continued answer" }]);
    const resumedKernel = new AgentKernel({ model: resumedModel, store, tools: [], maxSteps: 10 });

    // A run that already completed is terminal â€” not reported as active.
    const active = await resumedKernel.getActiveSessionIds();
    expect(active).toEqual([]);

    const resumedHandle = await resumedKernel.resumeRun(handle.runId, "Continue from here", testCtx);
    await resumedHandle.completed;

    const eventsAfter = await store.list(handle.runId);
    // Resume must NOT double-emit run.started or re-run the initial prompt.
    expect(eventsAfter.filter((e) => e.type === "run.started").length).toBe(1);
    expect(eventsAfter.filter((e) => e.type === "run.completed").length).toBe(2);

    const finalSnapshot = await store.getSnapshot(handle.runId);
    expect(finalSnapshot?.state.status).toBe("succeeded");
    expect(finalSnapshot?.state.finalOutput).toBe("Continued answer");
    expect(Number(finalSnapshot?.state.step ?? 0)).toBeGreaterThan(Number(snapshotBefore?.state.step ?? 0));
    expect(finalSnapshot?.state.sessionId).toBe("resume-sess-1");
  });

  it("TC04e: streamRun isolates concurrent runs â€” only events for its own runId", async () => {
  ...
  it("TC04b: maxSteps graceful â€” model produces text summary on last step, status succeeded", async () => {
  ...
  it("TC05: tool not found â€” records tool.failed event, run continues", async () => {
    const model = new FakeModelProvider([
      {
  ...
  it("TC06: tool execute throws error â€” records tool.failed, run continues", async () => {
  ...
  it("TC07: doom-loop â€” 3 identical tool calls detected and blocked", async () => {
  ...
  it("TC09: stream â€” kernel emits token.streamed events (live-only) when model supports streaming", async () => {
    const model = new FakeModelProvider([
  ...
  it("TC10: stream with tool calls â€” tool calls still work, token.streamed is live-only", async () => {
    const readFile = new FakeTool("read_file", async (input) => {
      return `Ná»™i dung: Hello World!`;
    });
  ...
  it("RV15: compaction persists a durable summary marker into SessionStore", async () => {
    const echo = new FakeTool("echo", async (input) => input);
    const model = new FakeModelProvider([
      { content: "", toolCalls: [{ id: "call-1", name: "echo", args: { msg: "first" } }] },
      { content: "Final answer" },
    ...
    const sessionStore = {
      async createSession() { return { id: "sess-rv15", title: "t", createdAt: "", updatedAt: "", isActive: true }; },
      async getSession() { return null; },
      async listSessions() { return []; },
      async updateSession() {},
      async deleteSession() {},
      async addMessage(_sid: string, role: string, content: string) {
        sessionMessages.push({ role, content });
      },
      async forkSession() { return { id: "s-fork", title: "t", createdAt: "", updatedAt: "", isActive: true }; },
      async listMessages() { return sessionMessages.map((m, i) => ({ id: `msg-${i}`, sessionId: "sess-rv15", role: m.role, content: m.content, createdAt: new Date().toISOString() })); },
    ...
    const compactor: ConversationCompactor = {
      async compact(msgs: readonly ChatMessage[]) {
        return {
          ...
          summary: { originalMessageCount: msgs.length, compressedMessageCount: 2, summary: "Compressed story so far." },
        };
      },
    };
    ...
    const kernel = new AgentKernel({
      ...
      maxSteps: 5, maxTokens: 10, compactor,
    });

    const handle = kernel.run("Hello! " + "x".repeat(2000), testCtx, "sess-rv15");
    await handle.completed;

    // The durable marker for the compacted context was written to the store.
    const markers = sessionMessages.filter((m) => m.role === "system" && m.content.includes("Compressed story so far."));
    expect(markers.length).toBeGreaterThan(0);
    // No compaction-summary marker on sessions without a run.
    expect(sessionMessages.some((m) => m.role === "system" && m.content.includes("[vnt-compaction-summary]"))).toBe(true);
  });

  it("RV15: compaction marker survives restart via restoreRunFromStore", async () => {
    const { restoreRunFromStore } = await import("@vinhnt-sdk/session");
    const { COMPACTION_SUMMARY_PREFIX } = await import("@vinhnt-sdk/schema");
    const eventStore = new FakeRunEventStore();
    await eventStore.append({ id: "e1" as never, runId: "run-rv15" as RunId, sequence: 0, type: "run.started", occurredAt: new Date().toISOString(), traceId: "t" as TraceId, data: {} });
    await eventStore.saveSnapshot("run-rv15", { sessionId: "sess-rv15", step: 3 });

    const messages = [
      { id: "m1", sessionId: "sess-rv15", role: "user" as const, content: "old prompt", createdAt: new Date().toISOString() },
      { id: "m2", sessionId: "sess-rv15", role: "assistant" as const, content: "old reply", createdAt: new Date().toISOString() },
      { id: "m3", sessionId: "sess-rv15", role: "system" as const, content: `${COMPACTION_SUMMARY_PREFIX}Everything pre-compaction was replaced.`, createdAt: new Date().toISOString() },
      { id: "m4", sessionId: "sess-rv15", role: "assistant" as const, content: "fresh reply", createdAt: new Date().toISOString() },
    ];
    const sessionStore = {
      async listMessages() { return messages; },
    } as unknown as SessionStore;

    const restored = await restoreRunFromStore(eventStore, sessionStore, "run-rv15" as RunId);
    expect(restored).not.toBeNull();
    const reloaded = restored!.sessionState.messages;
    expect(reloaded).toHaveLength(2);
    expect(reloaded[0]!.content).toBe("Everything pre-compaction was replaced.");
    expect(reloaded[1]!.content).toBe("fresh reply");
  });

  it("TC12: thinking step â€” emits thinking.* events (content live-only) and injects thinking into context", async () => {
    const model = new FakeModelProvider([
  ...
  it("TC13: thinking disabled (default) â€” does not emit thinking.* events", async () => {
  ...
  it("TC14: self-correct on tool failure â€” tool fails â†’ model fixes â†’ retry succeeds", async () => {
    let callCount = 0;
  ...
  it("TC15: self-correct disabled (default) â€” does not emit tool.self_correcting", async () => {
  ...
  it("TC08: abort() â€” cancel running run", async () => {
  ...
  it("TC16: session store receives model and tokens on successful run", async () => {
    ...
    // prompt "Test session" = 12 chars â†’ 3 tokens input
    // response "Hello session!" = 14 chars â†’ 4 tokens output
  ...
  it("TC23b (RV-36): run.completed + session stats written atomically when store supports appendTransactional", async () => {
    const model = new FakeModelProvider([{ content: "Hello!" }]);
    const store = new AtomicAwareStore();
    let updatesFromTransactional: Record<string, unknown> | undefined;
    const sessionStore = {
      async addMessage() {},
      async updateSession(_id: string, updates: Record<string, unknown>) {
        // If the atomic path is taken, this is never called directly â€” the
        // update goes through appendTransactional instead.
        updatesFromTransactional = updates;
      },
    } as unknown as SessionStore;

    const kernel = new AgentKernel({ model, store, tools: [], maxSteps: 10, sessionStore });

    const handle = kernel.run("Test atomic", testCtx, "session-atomic");
    await handle.completed;

    // appendTransactional was invoked (atomic write path), NOT separate writes.
    expect(store.transactionalCalls).toBeGreaterThan(0);
    expect(store.lastTransactionalSessionId).toBe("session-atomic");
    expect(store.lastTransactionalUpdates).toHaveProperty("model", "fake-model");

    // The event landed in the store and the session got its terminal stats.
    const completed = findEvent(await store.list(handle.runId), "run.completed");
    expect(completed?.data.status).toBe("succeeded");
    // Direct updateSession must NOT have been used (it would double-write).
    expect(updatesFromTransactional).toBeUndefined();
  });
  ...
  it("TC31b (RV-20): parent abort propagates to a running sub-agent â€” no orphan", async () => {
    let childModelAborted = false;
    const blockingModel: ModelProvider = {
      provider: "test",
      model: "blocking",
      contextLimit: undefined,
      capabilities: { streaming: true, toolCalling: true, imageInput: false, thinking: false, structuredOutput: false },
      pricing: { input: 0, output: 0 },
      generate: async (_req, signal) => new Promise<ModelResponse>((resolve) => {
        if (signal?.aborted) { childModelAborted = true; resolve({ content: "aborted" }); return; }
        signal?.addEventListener("abort", () => {
          childModelAborted = true;
    ...
    await agentRegistry.register(parent);
    await agentRegistry.register(child, parent.id);
    const kernel = new AgentKernel({ model: blockingModel, store, tools: [], maxSteps: 10, agentRegistry });
    await kernel.useAgent("p" as AgentId);

    const parentAbort = new AbortController();
    const outputPromise = kernel.runAgent(
      "c" as AgentId, "Work", testCtx, undefined, undefined, parentAbort.signal,
    );
    // Give the child run time to start its (blocking) model call.
    await new Promise((r) => setTimeout(r, 20));
    parentAbort.abort();

    const output = await outputPromise;
    expect(childModelAborted).toBe(true);
    ...
    function makeSpyPlugin(id: string, hooks: Partial<PluginHooks>): Plugin {
      return {
        ...
        hooks: {
          ...
          onToolInvoked: hooks.onToolInvoked ?? (async () => null),
          onToolCompleted: hooks.onToolCompleted ?? (async () => null),
  ...
  describe("wildcard permission matching", () => {
    it("TC37: allowedTools supports wildcard patterns", async () => {
      ...
      await handle.completed;
      const events = await store.list(handle.runId);
      // read_file matches read_* â†’ succeeds
  ...
  describe("permission ask flow", () => {
    it("ask with 'once' reply emits permission events then executes tool", async () => {
      ...
      const kernel = new AgentKernel({ model, store, tools: [readFile], maxSteps: 10, agentRegistry: registry, permissions: { approvalStore } });
...
describe("getRunOutput / getCompletedEventData snapshot optimization", () => {
  ...
  it("getCompletedEventData returns event data when available", async () => {
    ...
    expect((data as unknown as Record<string, unknown>).output).toBe("completed");
  });
  ...
  it("getCompletedEventData falls back to snapshot when no completed event after snapshot", async () => {
    ...
    const data = await getCompletedEventData(store, runId);
    expect(data).toBeTruthy();
    expect((data as unknown as Record<string, unknown>).output).toBe("snap-output");
  });
});
...
describe("P1-L: parallel run isolation (per-run context object)", () => {
  ...
  it("TC-P1L-01: two concurrent runs keep their own agent context", async () => {
    ...
    const handleA = kernel.createRunHandle("use probe", { ...testCtx, requestId: "p1l-a" as RequestId, traceId: "p1l-a" as TraceId }, "sess-a", undefined, alpha);
    const handleB = kernel.createRunHandle("use probe", { ...testCtx, requestId: "p1l-b" as RequestId, traceId: "p1l-b" as TraceId }, "sess-b", undefined, beta);
  ...
  it("TC-P1L-02: tools are resolved per run's agent, not the instance default", async () => {
    ...
    const handleA = kernel.run("alpha task", { ...testCtx, requestId: "p1l2-a" as RequestId, traceId: "p1l2-a" as TraceId }, "sess-a2", undefined, alpha);
    const handleB = kernel.run("beta task", { ...testCtx, requestId: "p1l2-b" as RequestId, traceId: "p1l2-b" as TraceId }, "sess-b2", undefined, beta);
...
describe("P1-M: single max-steps authority + per-run circuit breaker", () => {
  ...
  it("TC-P1M-02: circuit breaker state is per-run, not shared across runs", async () => {
    ...
    // The instance breaker must stay closed â€” it's no longer global state.
...
describe("P1-N: scrub env + redact log", () => {
  ...
  it("TC-P1N-02: kernel logger is redacted (model name secret scrubbed)", async () => {
    ...
    const secretModel: ModelProvider = {
      provider: "test",
      model: "test-model-sk-1234567890abcdef1234567890abcdef",
      contextLimit: undefined,
      capabilities: { streaming: true, toolCalling: true, imageInput: false, thinking: false, structuredOutput: false },
      pricing: { input: 0, output: 0 },
...
/**
 * FakeRunEventStore extended with an `appendTransactional` that records the
 * atomic event+session write (RV-36) while delegating everything else.
 */
class AtomicAwareStore extends FakeRunEventStore {
  transactionalCalls = 0;
  lastTransactionalSessionId: string | undefined;
  lastTransactionalUpdates: Record<string, unknown> | undefined;

  async appendTransactional(
    event: RunEvent,
    sessionUpdate?: { sessionId: string; updates: Record<string, unknown> },
  ): Promise<void> {
    this.transactionalCalls++;
    this.lastTransactionalSessionId = sessionUpdate?.sessionId;
    this.lastTransactionalUpdates = sessionUpdate?.updates;
