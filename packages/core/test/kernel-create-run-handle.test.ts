...
import type { AgentEvent, RunId, RequestId, TraceId } from "@vinhnt-sdk/schema";
...
describe("AgentKernel.createRunHandle", () => {
  ...
  it("RV7: cancel before first step lands -> single cancelled outcome + cancelled event flag", async () => {
    let modelCalls = 0;
    const slowModel: ModelProvider = {
      provider: "fake",
      model: "slow-model",
    ...
    const handle = kernel.createRunHandle("Run me", testCtx, "rv7-session");
    // Cancel synchronously — before the run loop's abort listener may attach.
    handle.cancel();

    const result = await handle.completed;
    expect(result.status).toBe("cancelled");
    // Whether or not the first step was entered, the model call must not run
    // to completion: at most one in-flight call is aborted.
    expect(modelCalls).toBeLessThanOrEqual(1);

    const events = await store.list(handle.runId);
    const completed = events.find((e) => e.type === "run.completed");
    expect(completed).toBeDefined();
    // The persisted event keeps the closed status enum but flags the cancel.
    expect((completed?.data as { cancelled?: boolean }).cancelled).toBe(true);

    const snapshot = await store.getSnapshot(handle.runId);
    expect(snapshot?.state.status).toBe("cancelled");
  });

  it("RV7: abort during an in-flight model call -> cancelled (never succeeded/failed)", async () => {
    const slowModel: ModelProvider = {
      provider: "fake",
      model: "slow-model",
      pricing: { input: 1.0, output: 2.0 },
    ...
    const handle = kernel.createRunHandle("Slow", testCtx);
    await new Promise((r) => setTimeout(r, 5));
    handle.cancel();
    ...
    const events = await store.list(handle.runId);
    const completed = events.find((e) => e.type === "run.completed");
    expect((completed?.data as { status?: string }).status).toBe("failed"); // enum unchanged
    expect((completed?.data as { cancelled?: boolean }).cancelled).toBe(true);
  });
