...
import type { ContextSourceKey, ContextSourceValue } from "../src/system-context/types.js";
...
function makeContextSource(id: string): ContextSourceValue {
  return {
    key: id as ContextSourceKey,
    async load() { return { value: id }; },
    renderBaseline(value: { value: string }) { return `context: ${value.value}`; },
    renderUpdate() { return null; },
    renderRemoval() { return `removed: ${id}`; },
  };
}
...
describe("DefaultPluginManager", () => {
  ...
  it("plugin can register context sources via context", async () => {
    ...
    expect(pm.contextSources[0]?.key).toBe("plugin-ctx" as ContextSourceKey);
  });
