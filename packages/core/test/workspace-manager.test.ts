...
describe("WorkspaceManager", () => {
  ...
  describe("detect", () => {
    ...
    it("detects root with .vnt/config.json", () => {
      vi.mocked(existsSync).mockImplementation((p) => String(p).includes(".vnt") || String(p).includes("package.json") || String(p).includes(".git"));
      vi.mocked(readdirSync).mockReturnValue([]);
    ...
    it("detects root with package.json", () => {
      vi.mocked(existsSync).mockImplementation((p) => {
        return String(p).endsWith("package.json") || String(p).endsWith(".git");
      });
      vi.mocked(readdirSync).mockReturnValue([]);
    ...
    it("detects subdirectory workspaces", () => {
      ...
      vi.mocked(existsSync).mockImplementation((p) => {
        return String(p).includes("pkg-a") || String(p).includes("pkg-b");
      });
      const results = WorkspaceManager.detect("/tmp/mono");
