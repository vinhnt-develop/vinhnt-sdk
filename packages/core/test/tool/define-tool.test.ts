...
describe("defineTool", () => {
  ...
  it("validates input at execution time", async () => {
    ...
    await expect(def.execute({} as never, noopCtx)).rejects.toThrow('Tool read_file input error: path');
  });
  ...
  it("runs normalize before validation", async () => {
    ...
    const def = tool.toDefinition();
    await expect(def.execute({ file_path: "/x" } as never, noopCtx)).resolves.toBe("/x");
  });
