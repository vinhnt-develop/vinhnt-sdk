...
const fakeRequire = (mod: unknown): RequireFunction => (() => mod) as unknown as RequireFunction;
...
function mockExecFileSuccess(stdout = "installed") {
  vi.mocked(execFile).mockImplementation(((
    ...
    _opts: unknown,
    cb: (error: Error | null, stdout: string, _stderr?: string) => void,
  ) => {
...
describe("loadPluginFromNpm", () => {
  ...
  it("installs package when not cached", async () => {
    ...
    mockExecFileSuccess();
    vi.mocked(createRequire).mockReturnValue(fakeRequire(createMockPlugin()));
  ...
  it("throws when package does not export a valid plugin", async () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(createRequire).mockReturnValue(fakeRequire({ notAPlugin: true }));
  ...
  it("loads default export from ESM package", async () => {
    ...
    vi.mocked(createRequire).mockReturnValue(fakeRequire({ default: mockPlugin }));
  ...
  it("handles non-object module export gracefully", async () => {
    vi.mocked(existsSync).mockReturnValue(true);
    vi.mocked(createRequire).mockReturnValue(fakeRequire(null));
...
describe("loadNpmPlugins", () => {
  ...
  it("loads and registers multiple plugins", async () => {
    ...
    vi.mocked(createRequire)
      .mockReturnValueOnce(fakeRequire(pluginA))
      .mockReturnValueOnce(fakeRequire(pluginB));
  ...
  it("handles individual plugin load failure without stopping others", async () => {
    ...
    vi.mocked(createRequire)
      .mockReturnValueOnce(fakeRequire(null))
      .mockReturnValueOnce(fakeRequire(goodPlugin));
