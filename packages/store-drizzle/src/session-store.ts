...
/**
 * SQLite-backed {@link SessionStore} via Drizzle ORM.
 *
...
export class DrizzleSessionStore implements SessionStore {
  ...
  async forkSession(sourceSessionId: string, title?: string): Promise<Session> {
    ...
    for (const msg of messages) {
      ...
      this.db.insert(MessageTable).values(vals as Parameters<typeof this.db.insert<typeof MessageTable>>[0]["$inferInsert"]).run();
    }
  ...
  async updateSession(id: string, updates: SessionUpdates): Promise<void> {
    this.db
      ...
      .set({
        ...
        ...(updates.inputTokens !== undefined ? { inputTokens: updates.inputTokens } : {}),
        ...(updates.outputTokens !== undefined ? { outputTokens: updates.outputTokens } : {}),
        ...(updates.agentId !== undefined ? { agentId: updates.agentId } : {}),
        ...(updates.location !== undefined
          ? {
              locationDirectory: updates.location.directory,
              ...(updates.location.workspaceId !== undefined
                ? { locationWorkspaceId: updates.location.workspaceId }
                : {}),
            }
          : {}),
      })
  ...
  async addMessage(
    ...
    if (toolCallId) vals.toolCallId = toolCallId;
    if (tokens) {
  ...
  async searchMessages(query: string, limit = 20): Promise<readonly Message[]> {
    try {
      ...
      // FTS unavailable — fall back to a LIKE scan
      const rows = this.db
        .select()
        .from(MessageTable)
...
function rowToSession(row: Record<string, unknown>): Session {
  return {
    ...
    createdAt: new Date(row.timeCreated as number).toISOString(),
    updatedAt: new Date(row.timeUpdated as number).toISOString(),
