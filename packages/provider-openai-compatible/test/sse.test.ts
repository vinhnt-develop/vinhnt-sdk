...
describe("toModelStreamEvents", () => {
  ...
  it("surfaces DeepSeek reasoning_content as thinking events (RV-44)", async () => {
    const input = [
      `data: ${JSON.stringify({
        id: "c",
        object: "chat.completion.chunk",
      ...
      `data: ${JSON.stringify({
        ...
        created: 1,
        model: "deepseek-reasoner",
        choices: [{ index: 0, delta: { reasoning_content: "solve this." }, finish_reason: null }],
      })}`,
      "",
      `data: ${JSON.stringify({
        id: "c",
    ...
    expect(thoughts.map((t) => (t as { content: string }).content)).toEqual(["Let me ", "solve this."]);
    expect(events).toContainEqual({ type: "text", content: "42" });
  });

  it("surfaces reasoning tokens from the final usage chunk (RV-44)", async () => {
    const input =
      `data: ${JSON.stringify({
        id: "u",
        object: "chat.completion.chunk",
    ...
    for await (const evt of toModelStreamEvents(toStream(input))) events.push(evt);
    const usage = events.find((e) => e.type === "usage");
    expect(usage).toEqual({ type: "usage", inputTokens: 10, outputTokens: 1000, reasoningTokens: 900 });
  });
