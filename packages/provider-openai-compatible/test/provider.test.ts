...
describe("OpenAICompatibleProvider.baseUrl validation (RV-50)", () => {
  it("normalizes trailing slashes on a valid http(s) URL", () => {
    const p = new OpenAICompatibleProvider({
      baseUrl: "https://api.example.com/v1/",
      apiKey: "k",
      defaultModel: "m",
    ...
    expect((p as unknown as { baseUrl: string }).baseUrl).toBe("https://api.example.com/v1");
  });

  it("throws ConfigurationError for a bare host (no protocol)", () => {
  ...
  it("throws ConfigurationError for a non-http(s) protocol", () => {
    expect(() => new OpenAICompatibleProvider({
...
describe("OpenAICompatibleProvider request timeout (RV-50)", () => {
  function hungProvider(opts: { timeoutMs: number; retry: RetryOptions }, fetchImpl: typeof fetch) {
    return new OpenAICompatibleProvider({
      baseUrl: "https://api.example.com/v1",
      apiKey: "k",
      defaultModel: "m",
  ...
  it("cuts off a never-settling fetch with TimeoutError", async () => {
    const hangingFetch = vi.fn(() => new Promise<Response>(() => {}));
    const p = hungProvider({ timeoutMs: 20, retry: { maxRetries: 0 } }, hangingFetch as unknown as typeof fetch);
  ...
  it("retries timeouts until retries are exhausted", async () => {
    const hangingFetch = vi.fn(() => new Promise<Response>(() => {}));
    const p = hungProvider(
      { timeoutMs: 20, retry: { maxRetries: 2, fixedBackoffMs: 1 } },
      hangingFetch as unknown as typeof fetch,
    );

    await expect(p.generate(request())).rejects.toMatchObject({ code: "TIMEOUT" });
    expect(hangingFetch).toHaveBeenCalledTimes(3);
  });
});

describe("OpenAICompatibleProvider includeUsage (RV-50)", () => {
  it("sends stream_options.include_usage by default when streaming", async () => {
    const fetchMock = vi.fn(async () => sseBody("data: [DONE]\n\n"));
    const p = provider(fetchMock as unknown as typeof fetch);
  ...
  it("omits stream_options when includeUsage is disabled at the provider level", async () => {
    const fetchMock = vi.fn(async () => sseBody("data: [DONE]\n\n"));
    const p = new OpenAICompatibleProvider({
      baseUrl: "https://api.example.com/v1",
      apiKey: "k",
  ...
  it("per-call streamOptions.includeUsage overrides the provider default", async () => {
    const fetchMock = vi.fn(async () => sseBody("data: [DONE]\n\n"));
    const p = provider(fetchMock as unknown as typeof fetch);

    for await (const _evt of p.stream(request({ streamOptions: { includeUsage: false } }))) { /* consume */ }
