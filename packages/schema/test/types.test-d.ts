...
describe('Schema Package Type Tests', () => {
  ...
  describe('Run Event Types', () => {
    ...
    it('KnownRunEvent should be a string', () => {
      expectTypeOf<KnownRunEvent['type']>().toBeString();
    });
  });
  ...
  describe('Core Types', () => {
    ...
    it('Result should have ok property', () => {
      expectTypeOf<Result<string, Error>>().toHaveProperty('ok');
    });
  ...
  describe('ModelProvider contract', () => {
    it('a provider without stream is a valid ModelProvider (stream is optional)', () => {
      ...
      expectTypeOf<ModelProvider['stream']>().toEqualTypeOf<
        ((request: ModelRequest, signal?: AbortSignal) => AsyncIterable<ModelStreamEvent>) | undefined
      >();
    });
  ...
  describe('ChatMessage role union (P2-C / gap G4)', () => {
    ...
    it('accepts a developer-role message', () => {
      ...
      expectTypeOf<Extract<ChatMessageRole, 'developer'>>().toEqualTypeOf<'developer'>();
    });
  });
