---
title: "Full Stack"
description: "NestJS integration example"
lang: "en"
type: "example"
category: "Examples"
sidebarPosition: 3
...
Complete agent platform with NestJS, REST API, SSE streaming, and WebSocket support.
...
## Key Concepts
- **`AgentService`** — NestJS injectable wrapping `AgentKernel` with `run` and `stream` methods.
- **`@Sse("stream")`** — Server-Sent Events endpoint for real-time streaming to the browser.
- **WebSocket Gateway** — Bidirectional real-time communication via Socket.IO.
- **`AgentModule`** — Modular NestJS structure for clean dependency injection.
